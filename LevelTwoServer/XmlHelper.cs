...
namespace GlobalData
{
    ...
    public class XmlHelper : IDisposable
    {
        ...
        public static string GetText(string xmlPath, string XmlPathNode)
        {
            string strText = "";
            ...
            strText = xnl[0].InnerText;
            return strText;
        }

        public static string GetText1(string xmlPath, string XmlPathNode)
        {
            string strText = "";
            DataTable dt = new DataTable();
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            XmlNodeList xnl = objXmlDoc.SelectNodes(XmlPathNode);
            strText = xnl[1].InnerText;
            return strText;
        }

        public static void XmlNodeReplaceXml(string xmlPath, string Node, string Content)
        {
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            objXmlDoc.SelectSingleNode(Node).InnerXml = Content;
            objXmlDoc.Save(xmlPath);
        }

        public static XmlNodeList GetXmlNodeList(string xmlPath, string XmlPathNode)
        {
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            XmlNodeList xnl = objXmlDoc.SelectNodes(XmlPathNode);
        ...
        static string strnode = "";
        public static string GetXmlList(string xmlPath, string XmlPathNode)
        {
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            XmlNodeList xnl = objXmlDoc.SelectNodes(XmlPathNode);
            strnode = xnl[0].InnerXml.Trim();
            //GetALLNode(xnl);
            return strnode;
        }

        public static DataTable GetXmlSubData(string xmlPath, string XmlPathNode)
        {
            DataTable dt = new DataTable();
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            ...
            if (xnl.Count > 0)
            {
                ...
                {
                    dt.Columns.Add(xnl[0].ChildNodes[i].Name);
                }
                dt.Rows.Add();
                for (int i = 0; i < xnl[0].ChildNodes.Count; i++)
                {
                    if (xnl[0].ChildNodes[i].Attributes.Count > 0)
                    {
                        dt.Rows[0][i] = xnl[0].ChildNodes[i].InnerText + "_" + xnl[0].ChildNodes[i].Attributes[0].InnerText;
                    }
                    else
                    {
                        dt.Rows[0][i] = xnl[0].ChildNodes[i].InnerText;
                    }
                }
            }
            return dt;
        ...
        public static DataTable GetXmlSubData3Axis(string xmlPath, string XmlPathNode)
        {
            DataTable dt = new DataTable();
            try
            {
                ...
                if (xnl.Count > 0)
                {
                    ...
                    dt.Columns.Add("Y");
                    dt.Columns.Add("Z");
                    for (int i = 1; i <= xnl[0].ChildNodes.Count; i++)
                    {
                        for (int j = 0; j < xnl[0].ChildNodes.Count; j++)
                        {
                            string strnum = xnl[0].ChildNodes[j].ChildNodes[0].ParentNode.Name.Replace("N", "").Trim();
                            if (strnum == i.ToString().Trim())
                            {
                                dt.Rows.Add();
                                dt.Rows[dt.Rows.Count - 1].RowError = strnum;
                                dt.Rows[dt.Rows.Count - 1][0] = (i);
                                dt.Rows[dt.Rows.Count - 1][1] = xnl[0].ChildNodes[j].ChildNodes[0].InnerText;
                                dt.Rows[dt.Rows.Count - 1][2] = xnl[0].ChildNodes[j].ChildNodes[1].InnerText;
                                dt.Rows[dt.Rows.Count - 1][3] = xnl[0].ChildNodes[j].ChildNodes[2].InnerText;
                                break;
                            }
                        }
                    }
        ...
        public static DataTable GetXmlData1(string xmlPath, string XmlPathNode)
        {
            DataTable dt = new DataTable();
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            ...
            if(xnl.Count > 0)
            {
                ...
                dt.Columns.Add("Name");
                dt.Columns.Add("Num");
                dt.Columns.Add("WeightCompensationOne");
                dt.Columns.Add("WeightCompensationTwo");
                dt.Columns.Add("TripCompensationOne");
                dt.Columns.Add("TripCompensationTwo");
                for (int i = 0; i < xnl[0].ChildNodes.Count;i++)
                {
                    dt.Rows.Add();
                    dt.Rows[dt.Rows.Count - 1][0] = xnl[0].ChildNodes[i].InnerText;
                    dt.Rows[dt.Rows.Count - 1][1] = xnl[0].ChildNodes[i].Attributes[0].InnerText;
                    dt.Rows[dt.Rows.Count - 1][2] = xnl[0].ChildNodes[i].Attributes[1].InnerText;
                    dt.Rows[dt.Rows.Count - 1][3] = xnl[0].ChildNodes[i].Attributes[2].InnerText;
                    dt.Rows[dt.Rows.Count - 1][4] = xnl[0].ChildNodes[i].Attributes[3].InnerText;
                    dt.Rows[dt.Rows.Count - 1][5] = xnl[0].ChildNodes[i].Attributes[4].InnerText;
                    dt.Rows[dt.Rows.Count - 1][6] = xnl[0].ChildNodes[i].Attributes[5].InnerText;
                }
            }
            return dt;
        }
        ...
        public static void XmlNodeReplace(string xmlPath, string Node, string Content,int intnum)
        {
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            objXmlDoc.SelectNodes(Node)[intnum].InnerText = Content;
            //objXmlDoc.SelectSingleNode(Node).InnerText = Content;
            objXmlDoc.Save(xmlPath);
        }

        public static void XmlChange(string xmlPath, string Node, string Content,int anum)
        {
            DataTable dt = new DataTable();
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            XmlNodeList xnl = objXmlDoc.SelectNodes(Node);
            if (xnl.Count > 0)
            {
                xnl[0].Attributes[anum].InnerText = Content;
            }
            objXmlDoc.Save(xmlPath);
        }
        #endregion
        ...
        public static void XmlInsertElementTwo(string xmlPath, string MainNode, string Element, string Attrib, string AttribContent, string Attrib1, string AttribContent1, string Content)
        {
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            XmlNode objNode = objXmlDoc.SelectSingleNode(MainNode);
        ...
        public static void XmlInsertElementFour(string xmlPath, string MainNode, string Element, string Attrib, string AttribContent, string Attrib1, string AttribContent1, string Attrib2, string AttribContent2, string Attrib3, string AttribContent3, string Attrib4, string AttribContent4, string Attrib5, string AttribContent5, string Attrib6, string AttribContent6, string Attrib7, string AttribContent7, string Content)
        {
            XmlDocument objXmlDoc = new XmlDocument();
            objXmlDoc.Load(xmlPath);
            XmlNode objNode = objXmlDoc.SelectSingleNode(MainNode);
            ...
            objElement.SetAttribute(Attrib5, AttribContent5);
            objElement.SetAttribute(Attrib6, AttribContent6);
            objElement.SetAttribute(Attrib7, AttribContent7);
            objElement.InnerText = Content;
            objNode.AppendChild(objElement);
            objXmlDoc.Save(xmlPath);
        }
